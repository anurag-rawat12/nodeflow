...
const GeminiSettingSchema = z.object({
...
type GeminiSettingForm = z.infer<typeof GeminiSettingSchema>;
...
const GeminiSetting = ({
    ...
    onSubmit?: (data: GeminiSettingForm) => void;
    variableName?: string;
    credentialId?: string;
    systemPrompt?: string;
    userPrompt?: string;
}) => {
    const form = useForm<GeminiSettingForm>({
        resolver: zodResolver(GeminiSettingSchema),
        defaultValues: {
                    ...
                    >
                        ...
                        <FormField
                            ...
                            render={({ field }) => {
                                ...
                                const geminiCreds = data?.filter((c) => c.type === "GEMINI") || []
                                        ...
                                        <FormControl>
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    ...
                                                    >
                                                        ...
                                                        <span className="text-sm truncate">
                                                            {selected?.key || "Select Gemini credential"}
                                                        </span>
                                                    </Button>
                                                </DropdownMenuTrigger>
                                                ...
                                                <DropdownMenuContent className="min-w-[220px]">
                                                    ...
                                                    >
                                                        {geminiCreds.length === 0 && (
                                                            <div className="px-3 py-2 text-xs text-muted-foreground">
                                                                No Gemini credentials found
                                                            </div>
                                                        )}

                                                        {geminiCreds.map((cred) => (
                                                            <DropdownMenuRadioItem
                                                                key={cred.id}
                                                                value={cred.id}
